class CurrencyConverter extends React.Component {
	...
	async getInfo() {
		const response = await fetch(
			wpApiSettings.root + 'alda-currency/v1/info'
		);
	...
	onUpdatePrimaryAmount( event ) {
		...
		for ( const key in this.state.currencyAmounts ) {
			newCurrencyAmounts[ key ] =
				this.state.currencyRates[ key ] * event.target.value;
		}
		this.setState( {
	...
	onUpdateCurrencyAmount( event ) {
		...
		const newBaseCurrencyAmount = (
			( this.state.initialBaseCurrencyAmount /
				parseFloat( this.state.currencyRates[ currency ] ) ) *
		...
		for ( const key in this.state.currencyAmounts ) {
			if ( key !== currency ) {
				newCurrencyAmounts[ key ] =
					parseFloat( this.state.currencyRates[ key ] ) *
					parseFloat( newBaseCurrencyAmount );
			}
		}
		this.setState( {
			...
			<table>
				<tbody>
					<tr>
						...
						<td>
							{ this.state.initialBaseCurrencyAmount.toFixed(
								2
							) }
						</td>
						<td>
					...
					{ Object.keys( this.state.currencyRates )
						.filter( ( k ) => k !== this.state.baseCurrency )
						.map( ( key ) => (
							<tr key={ key }>
								<th scope="row" data-currency={ key }>
									{ key }
								</th>
								...
								<td>
									<input
										...
										step="0.01"
										name={ key }
										value={ this.state.currencyAmounts[
											key
										].toFixed( 2 ) }
